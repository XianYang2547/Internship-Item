...
#include "seg_infer.h"
#include "seg_preprocess.h"
#include "seg_postprocess.h"
#include "seg_calibrator.h"
#include "seg_utils.h"
#include "seg_draw.h"
...
YoloSeg::YoloSeg(const std::string trtFile): trtFile_(trtFile)
{
...
std::vector<segDetection> YoloSeg::inference(cv::Mat& img){
    if (img.empty()) return {};
    ...
    std::vector<segDetection> vDetections;
...
void YoloSeg::draw_image(cv::Mat& img, std::vector<segDetection>& inferResult, bool drawBbox) {
    // Iterate over the detection results
    for (size_t i = 0; i < inferResult.size(); i++) {
        // Get the class name of the current detection
        std::string className = vClassNames[(int)inferResult[i].classId];

        // Check if the class should be drawn as a bounding box
        if (bboxClasses.find(className) != bboxClasses.end()) {
            // Draw bounding box
        ...
        // Check if the class should be drawn as a mask
        if (maskClasses.find(className) != maskClasses.end()) {
            // Draw mask (you may want to adjust this part based on how your masks are represented)
