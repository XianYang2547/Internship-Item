...
void display_video() {
    ...
    while (true) {
        ...
        1400, 0,    frame1.cols / 2.0,
        0,    1400, frame1.rows / 2.0,
        0,    0,    1);
    
        cv::Mat distCoeffs = (cv::Mat_<double>(1, 5) << -0.35, 0.15, 0.0005, 0.0005, -0.05);

        cv::Mat frame1_undistorted, frame2_undistorted;
        cv::undistort(frame1, frame1_undistorted, cameraMatrix, distCoeffs);
        cv::undistort(frame2, frame2_undistorted, cameraMatrix, distCoeffs);

        cv::resize(frame1_undistorted, frame1, cv::Size(960, 540));
        cv::resize(frame2_undistorted, frame2, cv::Size(960, 540));
