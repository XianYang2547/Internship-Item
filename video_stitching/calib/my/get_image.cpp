...
void display_video() {
    ...
    while (true) {
        ...
        if (cv::waitKey(1) == 's') {
            ...
            filename1 << "images/frame1_" << std::setw(3) << std::setfill('0') << save_count << ".png";
            filename2 << "images/frame2_" << std::setw(3) << std::setfill('0') << save_count << ".png";
            // 确保images目录存在
            system("mkdir -p images");  // Linux/macOS
            if (cv::imwrite(filename1.str(), frame1) && cv::imwrite(filename2.str(), frame2)) {
                std::cout << "成功保存第" << save_count << "组图片: " 
                          << filename1.str() << " 和 " << filename2.str() << std::endl;
            } else {
...
// g++ -o get_images get_image.cpp `pkg-config --cflags --libs opencv4`